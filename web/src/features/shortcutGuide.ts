  ...
  return `
    <article class="card muted">
      ...
      <ol class="steps">
        ...
        <li>添加「如果」：条件选上一步样本<strong>有任何值</strong>。在「如果」里面：先「计算统计数据」求<strong>总和</strong>，再添加「四舍五入」（0 位小数），再「设置变量」<code>kcal</code> 为该整数。合计若是 153,381 这种带逗号的数，「打开 URL」会把它拆成两个地址，就跳不回来。</li>
      ...
      <p>预览必须是纯数字热量，例如 <code>${baseUrl}#/sync/2026-09-18/153</code>，不能出现逗号。</p>
    </article>
  `;
}
