  ...
  return `
    <article class="card muted">
      ...
      <ol class="steps">
        ...
        <li>添加「文本」，内容为：<br /><code>输入#/sync/格式化日期/kcal</code><br />「输入」用快捷指令输入（本站地址），中间是 <code>yyyy-MM-dd</code> 日期，最后是 <code>kcal</code> 变量。不要用问号和 <code>&amp;</code>，否则「打开 URL」会把日期截掉，网页会提示缺少日期。</li>
        <li>添加「打开 URL」，URL 选上一步文本。预览应类似：<code>${baseUrl}#/sync/2026-09-18/387</code></li>
      </ol>
      <p>预览时若仍提示「尝试共享 N 个健康项目」：到「设置 → 快捷指令 → 高级」打开「允许共享大量数据」，并确认打开 URL 里只有数字、没有健康样本。</p>
      <p>本机预览时，快捷指令输入应是：</p>
